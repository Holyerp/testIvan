...
Optional flag (advanced):
- `/audit-pm --fix` — after producing the report, **offer** to apply low-risk fixes interactively (never silent). Not yet implemented in v1; the command currently only reports.
...
Present the report to the user and **ask** before acting:
...
**Do not auto-fix.** This command's job is discovery; fixes belong to a follow-up user-confirmed plan (see `/execute-work` + plan mode).
