...
export default function PurchaseInvoiceDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/purchase/invoices/${encodeURIComponent(id)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then(async (res) => {
        const json = await res.json();
        if (res.status === 404 || json.code === 'NOT_FOUND_PURCHASE_INVOICE') {
          const err = new Error('NOT_FOUND_PURCHASE_INVOICE');
          err.name = 'NOT_FOUND_PURCHASE_INVOICE';
          throw err;
        }
      ...
      .catch((err: Error) => {
        if (!active) return;
        if (err.name === 'NOT_FOUND_PURCHASE_INVOICE') {
          setNotFound(true);
        } else {
          setHasError(true);
        }
