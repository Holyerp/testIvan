...
interface AdvanceInvoiceHeader {
  id: string;
  number: string;
  vendorName: string;
  postingDate: string;
...
interface AdvanceInvoiceLineItem {
  description: string;
  quantity: number;
  unitPrice: number;
  vatPercent: number;
...
interface AdvanceInvoiceTotals {
  subtotal: number;
  vatAmount: number;
  total: number;
}
...
interface PurchaseAdvanceInvoiceDetail {
  header: AdvanceInvoiceHeader;
  lines: AdvanceInvoiceLineItem[];
  totals: AdvanceInvoiceTotals;
...
export default function PurchaseAdvanceInvoiceDetailPage() {
  const t = useTranslations('purchaseAdvanceDetail');
  ...
  const [detail, setDetail] = useState<PurchaseAdvanceInvoiceDetail | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/purchase/advance-invoices/${encodeURIComponent(id)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then(async (res) => {
        const json = await res.json();
        if (res.status === 404 || json.code === 'NOT_FOUND_PURCHASE_ADVANCE_INVOICE') {
          const err = new Error('NOT_FOUND_PURCHASE_ADVANCE_INVOICE');
          err.name = 'NOT_FOUND_PURCHASE_ADVANCE_INVOICE';
          throw err;
        }
        if (!res.ok || !json.success) throw new Error(json.code ?? 'INTEGRATION_BC_UNAVAILABLE');
        return json.data as PurchaseAdvanceInvoiceDetail;
      })
      .then((data) => {
        if (active) {
          setDetail(data);
      ...
      .catch((err: Error) => {
        if (!active) return;
        if (err.name === 'NOT_FOUND_PURCHASE_ADVANCE_INVOICE') {
          setNotFound(true);
        } else {
          setHasError(true);
        }
  ...
  return (
    <div className="p-8 font-sans">
      <Link
        href="/purchase/advance-invoices"
      ...
      {notFound ? (
        ...
        <>
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            {/* Advance tag distinguishes from a regular purchase invoice. */}
            ...
            >
              {tPurchase(`status.${detail.header.status}`)}
            </span>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-x-8 gap-y-4 rounded-xl border border-gray-100 bg-white p-6 shadow-sm sm:grid-cols-2">
            <Field label={t('vendor')} value={detail.header.vendorName} />
            <Field label={t('ourReference')} value={detail.header.ourReference} />
            <Field label={t('postingDate')} value={detail.header.postingDate} />
            <Field label={t('dueDate')} value={detail.header.dueDate} />
            <Field label={t('paymentTerms')} value={detail.header.paymentTerms} />
          </div>

          {/* Payment tracking */}
          <div className="mt-8">
            <h2 className="text-sm font-medium text-gray-500">{t('paymentTracking')}</h2>
            <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <PaymentCard label={t('amount')} value={detail.paymentTracking.amount} />
              <PaymentCard label={t('amountPaid')} value={detail.paymentTracking.amountPaid} accent="text-pine-green" />
              <PaymentCard label={t('remaining')} value={detail.paymentTracking.remaining} accent="text-amber-600" />
            </div>
          </div>
                  ...
                  {detail.lines.length === 0 ? (
                    ...
                    detail.lines.map((line, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right tabular-nums text-gray-700">{formatRsd(line.unitPrice)}</td>
                        <td className="px-4 py-3 text-right tabular-nums text-gray-700">{line.vatPercent}%</td>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-pine-navy">{value || '—'}</p>
    </div>
  );
}

function PaymentCard({ label, value, accent }: { label: string; value: number; accent?: string }) {
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
      <p className="text-xs font-medium uppercase tracking-wide text-gray-500">{label}</p>
      <p className={`mt-1 text-xl font-semibold tabular-nums ${accent ?? 'text-pine-navy'}`}>{formatRsd(value)}</p>
    </div>
  );
}

function TotalRow({ label, value, emphasized = false }: { label: string; value: number; emphasized?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      <span className={emphasized ? 'text-sm font-semibold text-pine-navy' : 'text-sm text-gray-500'}>{label}</span>
      <span
