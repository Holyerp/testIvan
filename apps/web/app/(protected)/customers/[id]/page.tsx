...
import { formatRsd, invoiceStatusKey } from '@/lib/format';
...
export default function CustomerDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/customers/${encodeURIComponent(id)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then(async (res) => {
        const json = await res.json();
        if (res.status === 404 || json.code === 'NOT_FOUND_CUSTOMER') {
          const err = new Error('NOT_FOUND_CUSTOMER');
          err.name = 'NOT_FOUND_CUSTOMER';
          throw err;
        }
      ...
      .catch((err: Error) => {
        if (!active) return;
        if (err.name === 'NOT_FOUND_CUSTOMER') {
          setNotFound(true);
        } else {
          setHasError(true);
        }
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-3xl font-semibold text-pine-navy tabular-nums">
                {formatRsd(detail.customer.balance)}
              </p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
              <p className="text-sm font-medium text-gray-500">{t('balanceDue')}</p>
              <p className="mt-2 text-3xl font-semibold text-pine-navy tabular-nums">
                {formatRsd(detail.customer.balanceDue)}
              </p>
            </div>
          </div>

          <div className="mt-8">
            <h2 className="text-sm font-medium text-gray-500">{t('recentInvoices')}</h2>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                    ...
                    <th className="px-4 py-3 font-medium">{t('postingDate')}</th>
                    ...
                    detail.invoices.map((inv) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right tabular-nums text-gray-700">
                          {formatRsd(inv.totalAmountIncludingTax)}
                        </td>
