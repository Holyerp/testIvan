...
interface CreditMemoTotalsData {
  subtotal: number;
  vatAmount: number;
  total: number;
}

interface SalesCreditMemoDetail {
  header: CreditMemoHeader;
  lines: CreditMemoLineItem[];
  totals: CreditMemoTotalsData;
}
...
export default function SalesCreditMemoDetailPage() {
  const t = useTranslations('creditMemoDetail');
  const tCreditMemos = useTranslations('creditMemos');
  const { user } = useRequireAuth(['ADMIN', 'MANAGER', 'ACCOUNTING']);
  const accessToken = useAuthStore((s) => s.accessToken);
  const params = useParams();
  const id = Array.isArray(params.id) ? params.id[0] : params.id;

  const [detail, setDetail] = useState<SalesCreditMemoDetail | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/sales/credit-memos/${encodeURIComponent(id)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then(async (res) => {
        const json = await res.json();
        if (res.status === 404 || json.code === 'NOT_FOUND_SALES_CREDIT_MEMO') {
          const err = new Error('NOT_FOUND_SALES_CREDIT_MEMO');
          err.name = 'NOT_FOUND_SALES_CREDIT_MEMO';
          throw err;
        }
        if (!res.ok || !json.success) throw new Error(json.code ?? 'INTEGRATION_BC_UNAVAILABLE');
        return json.data as SalesCreditMemoDetail;
      })
      .then((data) => {
        if (active) {
          setDetail(data);
      ...
      .catch((err: Error) => {
        if (!active) return;
        if (err.name === 'NOT_FOUND_SALES_CREDIT_MEMO') {
          setNotFound(true);
        } else {
          setHasError(true);
        }
      ...
      {notFound ? (
        ...
        <>
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            >
              {tCreditMemos(`status.${detail.header.status}`)}
            </span>
          </div>
