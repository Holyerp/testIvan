...
const TAB_PATHS: Record<Tab, string> = {
  open: '/api/v1/sales/invoices',
  posted: '/api/v1/sales/posted-invoices',
  creditMemos: '/api/v1/sales/credit-memos',
};
...
export default function SalesInvoicesPage() {
  ...
  return (
    <div className="p-8 font-sans">
      ...
      <SalesInvoiceTable key={tab} path={TAB_PATHS[tab]} />
    </div>
  );
}

function SalesInvoiceTable({ path }: { path: string }) {
  const t = useTranslations('sales');
  const router = useRouter();
  const today = new Date().toISOString().slice(0, 10);

  const query = usePaginatedQuery<SalesInvoiceListItem>({
    path,
    pageSize: 20,
    initialSortBy: 'date',
    initialSortDir: 'desc',
  ...
  return (
    <>
      ...
      <div className="mt-6">
        <EntityTable<SalesInvoiceListItem>
          columns={columns}
          rows={query.items}
          rowKey={(i) => i.id}
          ...
          onRowClick={(i) => router.push(`/sales/invoices/${i.id}`)}
