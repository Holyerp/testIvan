...
interface SalesAdvanceInvoiceDetail {
...
export default function SalesAdvanceInvoiceDetailPage() {
  const t = useTranslations('salesAdvanceDetail');
  ...
  const [detail, setDetail] = useState<SalesAdvanceInvoiceDetail | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/sales/advance-invoices/${encodeURIComponent(id)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then(async (res) => {
        const json = await res.json();
        if (res.status === 404 || json.code === 'NOT_FOUND_SALES_ADVANCE_INVOICE') {
          const err = new Error('NOT_FOUND_SALES_ADVANCE_INVOICE');
          err.name = 'NOT_FOUND_SALES_ADVANCE_INVOICE';
          throw err;
        }
        if (!res.ok || !json.success) throw new Error(json.code ?? 'INTEGRATION_BC_UNAVAILABLE');
        return json.data as SalesAdvanceInvoiceDetail;
      })
      .then((data) => {
        if (active) {
          setDetail(data);
      ...
      .catch((err: Error) => {
        if (!active) return;
        if (err.name === 'NOT_FOUND_SALES_ADVANCE_INVOICE') {
          setNotFound(true);
        } else {
          setHasError(true);
        }
      ...
      {notFound ? (
        ...
        <>
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            {/* Advance tag distinguishes from a regular sales invoice. */}
