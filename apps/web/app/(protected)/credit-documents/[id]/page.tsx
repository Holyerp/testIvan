...
interface CreditDocumentHeader {
  id: string;
  number: string;
...
interface CreditDocumentLineItem {
  description: string;
  quantity: number;
  unitPrice: number;
  vatPercent: number;
...
interface CreditDocumentTotalsData {
  subtotal: number;
  vatAmount: number;
  total: number;
}

interface CreditDocumentDetail {
  header: CreditDocumentHeader;
  lines: CreditDocumentLineItem[];
  totals: CreditDocumentTotalsData;
}
...
export default function CreditDocumentDetailPage() {
  const t = useTranslations('creditDocumentDetail');
  const tList = useTranslations('creditDocuments');
  const { user } = useRequireAuth(['ADMIN', 'MANAGER', 'ACCOUNTING']);
  const accessToken = useAuthStore((s) => s.accessToken);
  const params = useParams();
  const id = Array.isArray(params.id) ? params.id[0] : params.id;

  const [detail, setDetail] = useState<CreditDocumentDetail | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/credit-documents/${encodeURIComponent(id)}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then(async (res) => {
        const json = await res.json();
        if (res.status === 404 || json.code === 'NOT_FOUND_CREDIT_DOCUMENT') {
          const err = new Error('NOT_FOUND_CREDIT_DOCUMENT');
          err.name = 'NOT_FOUND_CREDIT_DOCUMENT';
          throw err;
        }
        if (!res.ok || !json.success) throw new Error(json.code ?? 'INTEGRATION_BC_UNAVAILABLE');
        return json.data as CreditDocumentDetail;
      })
      .then((data) => {
        if (active) {
          setDetail(data);
      ...
      .catch((err: Error) => {
        if (!active) return;
        if (err.name === 'NOT_FOUND_CREDIT_DOCUMENT') {
          setNotFound(true);
        } else {
          setHasError(true);
        }
      ...
      {notFound ? (
        ...
        <>
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            <span
              className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${creditDocumentTypeBadgeClass(detail.header.type)}`}
            >
              {tList(`type.${detail.header.type}`)}
            </span>
            <span
              className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${creditMemoStatusBadgeClass(detail.header.status)}`}
            >
              {tList(`status.${detail.header.status}`)}
            </span>
          </div>

          {/* Original-invoice reference — prominently shown, since a correction document
              only makes sense relative to the invoice it corrects. */}
          <div className="mt-4 inline-flex items-center gap-2 rounded-lg border border-pine-green/20 bg-pine-green/5 px-4 py-2 text-sm">
            <span className="font-medium text-gray-600">{t('originalInvoice')}:</span>
            <span className="font-semibold text-pine-navy">{detail.header.originalInvoiceNumber || '—'}</span>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-x-8 gap-y-4 rounded-xl border border-gray-100 bg-white p-6 shadow-sm sm:grid-cols-2">
            <Field label={t('type')} value={tList(`type.${detail.header.type}`)} />
            <Field label={t('party')} value={detail.header.partyName} />
            <Field label={t('postingDate')} value={detail.header.postingDate} />
            <Field label={t('status')} value={tList(`status.${detail.header.status}`)} />
          </div>
