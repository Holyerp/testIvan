...
public class PurchaseInvoiceDetailMapperTests
{
    private readonly PurchaseInvoiceDetailMapper _mapper = new();

    private static BcPurchaseInvoice SampleInvoice(string status = "Open") => new()
    {
        Id = "pinv001",
        Number = "PI-001",
        ...
        Status = status,
        PurchaseInvoiceLines = new List<BcPurchaseInvoiceLine>
        {
            new() { Description = "Sirovine", Quantity = 2m, UnitPrice = 1000m, VatPercent = 20m, LineAmount = 2000m },
            new() { Description = "Transport", Quantity = 1m, UnitPrice = 3000m, VatPercent = 20m, LineAmount = 3000m },
        },
    };
    ...
    public void Map_CopiesHeaderFields()
    {
        ...
        Assert.Equal("pinv001", dto.Header.Id);
        Assert.Equal("PI-001", dto.Header.Number);
        Assert.Equal("Supplier A d.o.o.", dto.Header.VendorName);
        ...
        Assert.Equal("REF-PI-001", dto.Header.OurReference);
    }
    ...
    public void Map_MapsLines()
    {
        ...
        Assert.Equal("Sirovine", first.Description);
    ...
    public void Map_NoLines_ProducesZeroTotals()
    {
        ...
        source.PurchaseInvoiceLines.Clear();
    ...
    {
        var source = SampleInvoice();
        source.PurchaseInvoiceLines = new List<BcPurchaseInvoiceLine>
        {
